...
PLAN="$ROOT/scripts/injection_plan_bzip.csv"
PLAN_RESULT="$ROOT/scripts/injection_plan_bzip_result.csv"
OUTCOME="$ROOT/scripts/outcome_bzip.txt"
...
$CMD
if [ $? -ne 0 ]; then
    echo "ERROR: Profiler failed. Check EXEC path and arguments."
    exit 1
fi
...
echo "Command: $CMD"
$CMD || { echo "ERROR: Incremental calculation failed"; exit 1; }

CMD="python3 $CALC_PY $INCREMENTAL_STAT"
echo "Command: $CMD"
$CMD || { echo "ERROR: FSS calculation failed"; exit 1; }
