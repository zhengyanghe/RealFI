...
PLAN="$ROOT/scripts/injection_plan_comd.csv"
PLAN_RESULT="$ROOT/scripts/injection_plan_comd_result.csv"
OUTCOME="$ROOT/scripts/outcome_comd.txt"
...
# =============================================
# Step 1: Run Profiler（每次都跑）
# =============================================
echo "[Step 1] Running profiler → $STAT"
setarch $(uname -m) -R pin -t $PIN_TOOL -o $STAT -sample_interval $SAMPLE_INTERVAL -- $EXEC $COMD_ARGS \
    >/dev/null 2>&1 || { echo "Profiler failed"; exit 1; }
echo "Profiler done"
