...
# remove rows with NaN in 'PC' or 'Execution_Count'
df = df[df['PC'].notnull() & df['Execution_Count'].notnull()]

# ensure Execution_Count is numeric and convert to int
df['Execution_Count'] = df['Execution_Count'].astype(int)

# --- key modification: calculate Exec_Diff ---
# group by 'PC' and calculate difference for 'Execution_Count'
# shift(1) replaces each row's value with the previous row's value (within the current group).
# fillna(df['Execution_Count']) is used to handle the first row of each PC group,
# its difference is its own Execution_Count (because it starts from 0)
df['Previous_Exec_Count'] = df.groupby('PC')['Execution_Count'].shift(1).fillna(0).astype(int)
...
df['Exec_Diff'] = df['Execution_Count'] - df['Previous_Exec_Count']
...
for idx in chosen_indices:
    ...
    # the final OCC is the previous Execution_Count + occ
