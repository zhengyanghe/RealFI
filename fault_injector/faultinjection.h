...
// 这里直接用 std::string，避免依赖 using namespace std 的位置
KNOB<std::string> instcount_file(
    KNOB_MODE_WRITEONCE, "pintool",
    "o", "pin.instcount.txt", "specify instruction count file name");

KNOB<std::string> fioption(
    KNOB_MODE_WRITEONCE, "pintool",
    "fioption", "", "specify fault injection option: all, sp, fp, ccs");

KNOB<std::string> fi_activation_file(
    KNOB_MODE_WRITEONCE, "pintool",
